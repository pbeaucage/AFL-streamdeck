...
def render_key_image(deck, key_style): #dict containing icon_filename, font_filename, label_text,icon_text):
    ...
    icon_font = ImageFont.truetype(key_style['icon_font'],30) #this must be one of only a few valid sizes for an emoji font: https://github.com/python-pillow/Pillow/issues/1422
...
def get_key_style(deck, key, state):
    ...
    if keyconfig['icon_font'] != '':
        ...
        keyconfig['icon_font'] = os.path.join(ASSETS_PATH,'NotoEmoji-Regular.ttf')
        
    if keyconfig['font'] != '':
        keyconfig['font'] = os.path.join(ASSETS_PATH,keyconfig['font'])
    else:
        keyconfig['font'] = os.path.join(ASSETS_PATH,'Quicksand-Regular.ttf')
...
def AFL_sd_queued_items(client):
    def cb(deck,key,state):
        try:
            ...
            queued = len(queue[2])+len(queue[1])
...
if __name__ == "__main__":
    ...
    keydata[10] = {'name':'robotpause','action':'state',
                  'Paused_appearance':{'icon':'','font':'FreeSans.ttf','label':'Robot\nPaused','icon_text':'🤖▶️','icon_font':'','icon_color':'red'},
                  'Error_appearance':{'icon':'','font':'FreeSans.ttf','label':'Connection\nError','icon_text':'🤖⚠️','icon_font':'','icon_color':'red'},
                  'Active_appearance':{'icon':'','font':'FreeSans.ttf','label':'Robot\nRunning','icon_text':'🤖⏸️','icon_font':'','icon_color':'green'},
                  'Ready_appearance':{'icon':'','font':'FreeSans.ttf','label':'Robot\nIdle','icon_text':'🤖⏸️','icon_font':'','icon_color':'yellow'},
    ...
    keydata[2] = {'name':'rinse','action':'momentary',
                   'appearance':{'icon':'','font':'FreeSans.ttf','label':'RinseCell','icon_text':'💦','icon_font':'','icon_color':'blue'},
                   'callback':AFL_sd_enqueue(load_client,'rinseCell')}
    keydata[3] = {'name':'load','action':'momentary',
                   'appearance':{'icon':'','font':'FreeSans.ttf','label':'Load\nSample','icon_text':'➡️','icon_font':'','icon_color':'red'},
                   'callback':AFL_sd_enqueue(load_client,'loadSample')}
    ...
    keydata[12] = {'name':'refill','action':'momentary',
              'appearance':{'icon':'','font':'FreeSans.ttf','label':'Refill\nTipracks','icon_text':'💉','icon_font':''},
              'callback':AFL_sd_enqueue(robot_client,'reset_tipracks')
             }
    keydata[13] = {'name':'home','action':'momentary',
              'appearance':{'icon':'','font':'FreeSans.ttf','label':'Home\nRobot','icon_text':'🏠','icon_font':''},
              'callback':AFL_sd_enqueue(robot_client,'home')
             }
    keydata[1] = {'name':'loaderqueue','action':'momentary',
              'appearance':{'icon':'','font':'FreeSans.ttf','label':'queued','icon_text':'4','icon_font':'FreeSans.ttf'},
              'callback':dummy_cb,
              'status_type':'numeric',
              'status_callback':AFL_sd_queued_items(load_client)
             }
    keydata[6] = {'name':'instqueue','action':'momentary',
              'appearance':{'icon':'','font':'FreeSans.ttf','label':'queued','icon_text':'2','icon_font':'FreeSans.ttf'},
              'callback':dummy_cb,
              'status_type':'numeric',
              'status_callback':AFL_sd_queued_items(inst_client)
             }
    keydata[11] = {'name':'robotqueue','action':'momentary',
              'appearance':{'icon':'','font':'FreeSans.ttf','label':'queued','icon_text':'0','icon_font':'FreeSans.ttf'},
              'callback':dummy_cb,
              'status_type':'numeric',
